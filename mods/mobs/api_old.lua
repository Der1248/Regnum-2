...
function mob_class:set_velocity(v)
	...
	local s = self.object:get_velocity()
    if s == nil then
        return
    end
...
function mob_class:mob_activate(staticdata, def, dtime)
    ...
    if self.name == "mobs:knight" then
        self.object:set_properties({infotext=self.owner.."'s Knight"})
        if self.owner then
            if self.metadata == 1 then
			    self.object:set_properties({textures={"mobs_knight_red.png"}})
		    end
		    if self.metadata == 2 then
			    self.object:set_properties({textures={"mobs_knight_blue.png"}})
		    end
		    if self.metadata == 3 then
			    self.object:set_properties({textures={"mobs_knight_green.png"}})
		    end
		    if self.metadata == 4 then
			    self.object:set_properties({textures={"mobs_knight_pink.png"}})
		    end
            if self.metadata == 5 then
			    self.object:set_properties({textures={"mobs_knight_purple.png"}})
		    end
            if self.metadata2 == 1 then
				self.walk_velocity = 1
				self.run_velocity = 3
    ...
    if self.metadata3 > 1 then
		...
		if self.name == "mobs:knight" then
			if self.owner and minetest.get_player_by_name(self.owner) then
				...
				minetest.get_player_by_name(self.owner):set_attribute("knight2_meta1", ""..self.metadata)
				minetest.get_player_by_name(self.owner):set_attribute("knight2_meta2", ""..self.metadata2)
			end
...
function update(self, clicker)
    if self.name == "mobs:knight" then
		if self.owner ~= clicker:get_player_name() then
			...
			minetest.show_formspec(player_name, "npcf_"..self.owner.."'s Knight2", formspec)
		else
			local inv = minetest.get_player_by_name(self.owner):get_inventory()
			inv:set_size("knight",8)
			local player_name = clicker:get_player_name()
			local formspec = "size[8,8.5]"
			.."list[current_player;knight;0,2;8,1]"
			.."list[current_player;main;0,4.5;8,4]"
            .."listring[current_player;knight]"
            ...
            if self.metadata == 1 then
			formspec = formspec .."button[2,0;2.0,0.5;texture11;Texture:blue]"
			end
			if self.metadata == 2 then
			formspec = formspec .."button[2,0;2.0,0.5;texture12;Texture:green]"
			end
            if self.metadata == 3 then
			formspec = formspec .."button[2,0;2.0,0.5;texture13;Texture:pink]"
			end
            if self.metadata == 4 then
			formspec = formspec .."button[2,0;2.0,0.5;texture14;Texture:purple]"
			end
            if self.metadata == 5 then
			formspec = formspec .."button[2,0;2.0,0.5;texture15;Texture:red]"
			end
			minetest.show_formspec(player_name, "npcf_"..self.owner.."'s Knight2", formspec)
		end
	end
    ...
    on_receive_fields = function(self, fields, sender)
        if self.owner == sender:get_player_name() then
            ...
            elseif fields.texture12 then
			    ...
			    self.metadata = 3
                update(self, sender)
            elseif fields.texture13 then
			    self.object:set_properties({textures={"mobs_knight_pink.png"}})
			    self.metadata = 4
                update(self, sender)
            elseif fields.texture14 then
			    self.object:set_properties({textures={"mobs_knight_purple.png"}})
			    self.metadata = 5
                update(self, sender)
            elseif fields.texture15 then
			    self.object:set_properties({textures={"mobs_knight_red.png"}})
			    self.metadata = 1
                update(self, sender)
